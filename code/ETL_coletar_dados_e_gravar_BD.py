...
engine = create_engine('postgresql://'+user+':'+passw +
                       '@'+host+':'+port+'/'+database)
conn = psycopg2.connect('dbname='+database+' '+'user=' +
                        user+' '+'host='+host+' '+'password='+passw)
cur = conn.cursor()
...
for e in range(0, len(arquivos_estabelecimento)):
    ...
    extracted_file_path = Path(
        f'{extracted_files}/{arquivos_estabelecimento[e]}')
    ...
    # Gravar dados no banco:
    # estabelecimento
    estabelecimento.to_sql(name='estabelecimento',
...
for e in range(0, len(arquivos_simples)):
    ...
    for i in range(0, partes):
        ...
        # Gravar dados no banco:
        # simples
        simples.to_sql(name='simples', con=engine,
...
for e in range(0, len(arquivos_cnae)):
    ...
    cnae = pd.read_csv(filepath_or_buffer=extracted_file_path, sep=';',
                       skiprows=0, header=None, dtype='object', encoding='ANSI')
